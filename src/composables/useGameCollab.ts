...
export function useGameCollab(room: Ref<string>, playerId: Ref<string>) {
  ...
  const yjsUrl = import.meta.env.VITE_YJS_URL || `${websocketProtocol()}://${websocketHost()}`
  const provider = new WebsocketProvider(yjsUrl, room.value, doc)
