...
import DamageLeaderboardPanel from './components/DamageLeaderboardPanel.vue'
import CombatEventsPanel from './components/CombatEventsPanel.vue'
import GameSavesPanel from './components/GameSavesPanel.vue'
import TestMonsterPanel from './components/TestMonsterPanel.vue'
...
import { useGameScene } from './composables/useGameScene'
...
import { formatMonsterHp, formatMonsterSpeed, monsterAbilityEntries } from './game/monsters'
import { formatAttackIntervalSeconds, isAllyBuffAuraAbility, towerData, towerDisplayName } from './game/towers'
import type { BaseTowerId, BattleSnapshot } from './game/types'

const { room, playerId, online } = useGameRoom()
...
} = useGameCollab(room, playerId, online)
...
function closeInfoPanels() {
  const panels = [showMoreTools, showLayouts, showLeaderboard, showCombatEvents, showSaves, showTestMonster]
  const closed = panels.some((panel) => panel.value)
  panels.forEach((panel) => { panel.value = false })
  return closed
}
  ...
  <main class="game-shell">
    ...
    >
      <nav class="game-float-tools" aria-label="战场工具" @pointerdown.stop @pointerup.stop>
        ...
        <button class="float-tool primary-tool" @click="toggleCameraView">{{ topDownView ? '3D' : '俯视' }}</button>
        <button
      ...
      <TestMonsterPanel
        v-model:visible="showTestMonster"
        :battle-connected="battleConnected"
        :spawn-test-monster="spawnTestMonster"
        :show-message="showPlacementMessage"
      />

      <DamageLeaderboardPanel
        v-model:visible="showLeaderboard"
        v-model:wave="leaderboardWave"
        :leaderboard-waves="leaderboardWaves"
        :leaderboard-entries="leaderboardEntries"
        :has-leaderboard-entries="hasLeaderboardEntries"
        :leaderboard-entry-label="leaderboardEntryLabel"
      />

      <CombatEventsPanel
        v-model:visible="showCombatEvents"
        :combat-events="combatEvents"
        :has-combat-events="hasCombatEvents"
      />

      <GameSavesPanel
        v-model:visible="showSaves"
        :saves="saves"
        :has-saves="hasSaves"
        :save-loading-id="saveLoadingId"
